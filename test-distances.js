...
const tlssr = "13.7438,100.5583"; // TLSSR
...
async function run() {
  console.log("=== 1. Branch -> Customer ===");
  const d1 = await getRoute("TLSS1 -> 19 At Chidlom", tlss1, customer);
  const d2 = await getRoute("TLSSR -> 19 At Chidlom", tlssr, customer);
  console.log(`Closer from Branch to Customer: ${d1 < d2 ? 'TLSS1' : 'TLSSR'}`);
  
  console.log("\n=== 2. Customer -> Branch ===");
  const d3 = await getRoute("19 At Chidlom -> TLSS1", customer, tlss1);
  const d4 = await getRoute("19 At Chidlom -> TLSSR", customer, tlssr);
  console.log(`Closer from Customer to Branch: ${d3 < d4 ? 'TLSS1' : 'TLSSR'}`);
}
