...
export async function generateA5ReceiptImage(
  ...
  if (logoImg) {
    ...
    ctx.drawImage(logoImg, padding, y, Math.min(logoWidth, 180), logoHeight);
    y += logoHeight + 12;
  }
  ...
  // Wrap shop name if it's too long (matches dialog word-wrap behavior)
  ...
  // Wrap address across multiple lines cleanly (max 45 chars per line)
  const words = shopAddress.split(" ");
  let line = "";
  for (let i = 0; i < words.length; i++) {
    const testLine = line + words[i] + " ";
    if (ctx.measureText(testLine).width > 260 && i > 0) {
  ...
  // Customer Section (Billed To) — match dialog layout (no COLLECTION DATE)
  const custY = y;
  ctx.textAlign = "left";
  ...
  ctx.fillStyle = "#111827";
  ctx.fillText(receiptData.customerName || "Walk-In Guest", padding, custY + 20);

  ctx.font = "12px sans-serif";
  ctx.fillStyle = "#4b5563";
  ctx.fillText(receiptData.customerPhone || "-", padding, custY + 36);

  // Note: COLLECTION DATE is intentionally omitted here to match the dialog display
  ...
  ctx.fillStyle = "#111827";
  ctx.textAlign = "left";
  ctx.fillText("DESCRIPTION", padding + 5, y + 20);
  ctx.textAlign = "center";
  ctx.fillText("QTY", width - padding - 280, y + 20);
  ctx.textAlign = "right";
  ctx.fillText("UNIT PRICE", width - padding - 130, y + 20);
  ctx.fillText("TOTAL", width - padding - 5, y + 20);
  ...
  receiptData.items.forEach((item) => {
    ...
    ctx.fillText(name, padding + 5, y + 22);

    ctx.textAlign = "center";
    ctx.fillText(String(item.quantity), width - padding - 280, y + 22);

    ctx.textAlign = "right";
    ctx.fillText(formatCurrency(item.price), width - padding - 130, y + 22);
    ctx.fillText(formatCurrency(item.price * item.quantity), width - padding - 5, y + 22);
  ...
  if (hasDeliveryFee) {
    ctx.fillStyle = "#1f2937";
    ctx.textAlign = "left";
    ctx.fillText("Delivery Fee", padding + 5, y + 22);
    ctx.textAlign = "center";
    ctx.fillText("1", width - padding - 280, y + 22);
    ctx.textAlign = "right";
    ctx.fillText(formatCurrency(receiptData.deliveryFee || 0), width - padding - 130, y + 22);
    ctx.fillText(formatCurrency(receiptData.deliveryFee || 0), width - padding - 5, y + 22);
    y += 36;
  ...
  if (hasExpress) {
    ...
    ctx.textAlign = "left";
    ctx.fillText(`Express Surcharge (${receiptData.serviceSpeed === "express_50" ? "+50%" : "+100%"})`, padding + 5, y + 22);
    ctx.textAlign = "center";
    ctx.fillText("1", width - padding - 280, y + 22);
    ctx.textAlign = "right";
    ctx.fillText(formatCurrency(receiptData.expressSurcharge), width - padding - 130, y + 22);
    ctx.fillText(formatCurrency(receiptData.expressSurcharge), width - padding - 5, y + 22);
    y += 36;
  ...
  if (hasDiscount) {
    ...
    ctx.textAlign = "left";
    ctx.fillText(`Discount ${receiptData.discountPercent ? `(${receiptData.discountPercent}%)` : ""}`, padding + 5, y + 22);
    ctx.textAlign = "center";
    ctx.fillText("1", width - padding - 280, y + 22);
    ctx.textAlign = "right";
    ctx.fillText(`-${formatCurrency(receiptData.discount)}`, width - padding - 130, y + 22);
    ctx.fillText(`-${formatCurrency(receiptData.discount)}`, width - padding - 5, y + 22);
    y += 36;
  ...
  if (receiptData.status === "cancel") {
    ...
    ctx.lineWidth = 4;
    ctx.font = "900 18px sans-serif";
    ctx.strokeRect(width - padding - 140, footerY - 70, 140, 48);
    ctx.textAlign = "center";
    ctx.fillText("VOIDED", width - padding - 70, footerY - 38);
  }
  ...
  ctx.fillText(footerText, width / 2, footerY + 10);
