...
async function main() {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const pickupCommission = customer.isVIP ? 0 : Math.floor(pickupDist * 2) * 2;
    const deliveryCommission = customer.isVIP ? 0 : Math.floor(deliveryDist) * 2;
    ...
    let pRiderId: string | null = null;
    let dRiderId: string | null = null;
    let isPickupCompleted = false;
    let isDeliveryCompleted = false;

    let pOutStatus = "pending";
    let pInStatus = "pending";
    let dOutStatus = "pending";
    let dInStatus = "pending";
    ...
    if (isPickupToday) {
      ...
      if (pickupTime.getHours() < 15) {
        ...
        pOutStatus = "completed";
        pInStatus = "completed";
      } else {
        status = "pickup";
        pOutStatus = "in_transit";
      }
    } else {
      ...
      if (deliveryTime.getHours() < 15) {
        ...
        dOutStatus = "completed";
        dInStatus = "completed";
      } else {
        status = "delivery";
        dOutStatus = "in_transit";
      }
    }
    ...
    const legsJson = JSON.stringify({
      pickupOutbound: { riderId: pRiderId, scheduledAt: pickupTime, status: pOutStatus },
      pickupInbound: { riderId: pRiderId, scheduledAt: pickupTime, status: pInStatus },
      deliveryOutbound: { riderId: dRiderId, scheduledAt: deliveryTime, status: dOutStatus },
      deliveryInbound: { riderId: dRiderId, scheduledAt: deliveryTime, status: dInStatus }
    });
  ...
  for (const jobData of jobsToCreate) {
    ...
    if (isPickupCompleted && job.pickupRiderId) {
      ...
      if (rider) {
        ...
        const vipTag = isVIP ? " [VIP: ไม่มีค่าคอม]" : "";
        await prisma.riderTransaction.create({
          data: {
            ...
            detail: `Job ${job.id} - รับผ้า (${job.pickupDistance}km)${vipTag}`
          }
        });
      }
    }
      ...
      if (rider) {
        ...
        await prisma.riderTransaction.create({
          data: {
            ...
            detail: `Job ${job.id} - ส่งผ้า (${job.deliveryDistance}km)${vipTag}`
          }
        });
      }
    }
  ...
  console.log("✅ Successfully seeded 200 jobs with correct legsJson, VIP commission logic, and 0-THB Transactions!");
}
