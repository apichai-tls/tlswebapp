...
async function main() {
  ...
  for (const jobData of jobsToCreate) {
    ...
    if (isPickupCompleted && job.pickupRiderId) {
      ...
      if (rider && job.pickupCommission! > 0) {
        ...
        rider.completedJobs += 1; // Still count as completed job even if 0 commission
      }
    }

    if (isDeliveryCompleted && job.deliveryRiderId) {
      const rider = riderMap.get(job.deliveryRiderId);
      if (rider && job.deliveryCommission! > 0) {
  ...
  console.log("✅ Successfully seeded 200 jobs with correct legsJson and VIP commission logic!");
}
